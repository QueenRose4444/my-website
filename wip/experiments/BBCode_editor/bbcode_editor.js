...
function syncLog(...args) {
  if (LOGGING_ENABLED) console.log("[SYNC_LOG]", ...args);
}
...
function getElements() {
  return {
    ...
    patchNotesVersionToggle: document.getElementById(
      "patchnotes-version-toggle",
    ),
...
function migrateGameData(game) {
  if (
    game.urlGroups &&
    (!game.customGroups || game.customGroups.length === 0)
  ) {
    game.customGroups = game.urlGroups.map((ug) => ({
      title: ug.title || "Proton Drive Links",
      footer: "",
      files: ug.files || [],
    }));
  }
  ...
  const ensureFileProps = (files) => {
    ...
    return files.map((f) => {
      ...
      if (f.crackType === undefined)
        f.crackType = "Cracked: Detanup01 Goldberg Fork";
  ...
  game.customGroups.forEach((grp) => {
    grp.files = ensureFileProps(grp.files);
  });
  ...
  if (game.files && game.customGroups) {
    game.customGroups.forEach((grp) => {
      grp.files.forEach((cgFile) => {
        const mainFile = game.files.find(
          (mf) =>
            mf.platform === cgFile.platform && mf.branch === cgFile.branch,
        );
        if (mainFile) {
          ...
          if (!cgFile.crackedFileSize2 && mainFile.crackedFileSize2) {
            cgFile.crackedFileSize2 = mainFile.crackedFileSize2;
          }
        }
      });
    });
  ...
  if (game.mainGroupTitle === undefined)
    game.mainGroupTitle = "Proton Drive Links";
...
function loadLocalData() {
  ...
  const defaultSettings = {
    ...
    showPatchNotesVersionLabel: true,
  };
  try {
    ...
    if (storedData) {
      ...
      if (parsedData.games)
        parsedData.games = parsedData.games.map(migrateGameData);
      state = {
        ...state,
        ...parsedData,
...
const parseBBCodeInput = (text) => {
  ...
  const pnMatch = patchNotesBody.match(
    /\[size=88\]\[color=white\]\[b\]\s*(.*?)\s*\[/,
  );
  const patchNotesTitle =
    pnMatch && !pnMatch[1].includes("Version") ? pnMatch[1].trim() : "";
  ...
  const extractFilesFromBlock = (block) => {
    ...
    const urlLineRegex =
      /\[url=([^\]]*?)\]\[color=.*?\]\[b\](.*?) \[(.*?)\] \[Branch: (.*?)\] \((.*?)\)\[\/b\]\[\/color\]\[\/url\]/g;
    const dateRegex =
      /\[size=85\].*?Version:\[\/b\] \[i\](.*?) \[Build (.*?)\]\[\/i\]/;
  ...
  const gameTitle =
    mainFiles.length > 0
      ? mainFiles[0].gameTitle
      : customGroups[0]?.files[0]?.gameTitle || "Imported Game";
...
const mergeGameData = (existing, incoming) => {
  ...
  if (incoming.patchNotesTitle)
    merged.patchNotesTitle = incoming.patchNotesTitle;
  if (
    incoming.mainGroupTitle &&
    incoming.mainGroupTitle !== "Proton Drive Links"
  )
    merged.mainGroupTitle = incoming.mainGroupTitle;
  ...
  const unionMergeFiles = (targetList, incomingList) => {
    ...
    const result = targetList.map((existingFile) => {
      ...
      const fresh = incomingList.find(
        (f) =>
          f.platform === existingFile.platform &&
          f.branch === existingFile.branch,
      );
      if (fresh) {
    ...
    incomingList.forEach((fresh) => {
      const exists = result.some(
        (r) => r.platform === fresh.platform && r.branch === fresh.branch,
      );
      if (!exists) {
        result.push(fresh);
      }
  ...
  if (incoming.customGroups && incoming.customGroups.length > 0) {
    incoming.customGroups.forEach((ig) => {
      const existingGroupIndex = merged.customGroups.findIndex(
        (eg) => eg.title === ig.title,
      );
      if (existingGroupIndex !== -1) {
        // Merge overlapping group
        merged.customGroups[existingGroupIndex].files = unionMergeFiles(
          merged.customGroups[existingGroupIndex].files,
          ig.files,
        );
        merged.customGroups[existingGroupIndex].footer =
          ig.footer || merged.customGroups[existingGroupIndex].footer;
      } else {
        // Add new group
  ...
  if (incoming.updates && incoming.updates.length > 0) {
    ...
    merged.updates = incoming.updates.map((iu) => {
      ...
      const eu = existing.updates
        ? existing.updates.find((u) => u.title === iu.title)
        : null;
...
const parseInputText = (text) => {
  ...
  const rawBlockRegex =
    /\[url=.*?\](?:\[color=.*?\])?\[b\](?<gameName>.+?) \[(?<platform>Win\d+|Linux\d+|Mac)\] \[Branch: (?<branch>[^\]]+)\] \(Clean Steam Files\)\[\/b\][\s\S]*?Version:\[\/b\]\s*\[i\](?<fullDate>.+?)\s*\[Build\s(?<buildId>\d+)\]\[\/i\]/g;
  ...
  if (text.includes("[spoiler=") && text.includes("Branch:")) {
    ...
    if (bbGame) {
      const existIdx = state.games.findIndex(
        (g) => g.gameTitle === bbGame.gameTitle,
      );
      if (existIdx !== -1) {
  ...
  while ((match = rawBlockRegex.exec(text)) !== null) {
    ...
    // Find existing file
    const existingFileIndex = currentGame.files.findIndex(
      (f) => f.platform === platform && f.branch === branch,
    );
  ...
  for (const [title, game] of gamesToUpdate.entries()) {
    ...
    if (game.customGroups)
      game.customGroups.forEach((g) => g.files.sort(sorter));

    gamesMap.set(title, game);
  }

  if (gamesToUpdate.size > 0) {
    state.games = Array.from(gamesMap.values()).sort((a, b) =>
...
const finishImport = (targetTitle) => {
  ...
  localStorage.setItem(
    `${STORAGE_PREFIX}activeGameIndex`,
...
// --- RENDER ENGINE ---
const renderOutput = () => {
  const { outputCode, previewPane, copyBtnTop, copyBtnBottom, downloadBtn } =
    getElements();
  if (state.games.length === 0) {
    outputCode.value = "";
    previewPane.innerHTML = '<p class="text-gray-500">No data.</p>';
    [copyBtnTop, copyBtnBottom, downloadBtn].forEach(
      (b) => (b.disabled = true),
    );
    return;
  }
  ...
  if (activeGame.customGroups)
    activeGame.customGroups.forEach((grp) => grp.files.sort(platformSorter));

  let template = state.template;

  // Replace Top Level SHOW_VERSION_LABEL
  template = template.replace(
    /<!--IF:showVersionLabel-->([\s\S]*?)<!--\/IF:showVersionLabel-->/g,
    state.settings.showVersionLabel ? "$1" : "",
  );

  template = template.replace(
    /<!--IF:gameVersion-->([\s\S]*?)<!--\/IF:gameVersion-->/g,
    activeGame.gameVersion ? "$1" : "",
  );
  template = template.replace(
    /<!--IF:crackedExists-->([\s\S]*?)<!--\/IF:crackedExists-->/g,
    activeGame.files.some((f) => f.includeCracked) ? "$1" : "",
  );
  template = template.replace(
    /<!--IF:customGroups-->([\s\S]*?)<!--\/IF:customGroups-->/g,
    activeGame.customGroups && activeGame.customGroups.length > 0 ? "$1" : "",
  );
  template = template.replace(
    /<!--IF:endNote-->([\s\S]*?)<!--\/IF:endNote-->/g,
    activeGame.endNote ? "$1" : "",
  );

  const processLoops = (tmpl, context, parentContext = {}) => {
    return tmpl.replace(
      /<!--LOOP:(\w+)-->([\s\S]*?)<!--\/LOOP:\1-->/g,
      (match, loopKey, loopContent) => {
        let items = [];
        if (loopKey === "cleanFiles") items = context.files;
        else if (loopKey === "crackedFiles")
          items = context.files.filter((f) => f.includeCracked);
        else if (loopKey === "crackedFiles2")
          items = context.files.filter((f) => f.includeCracked2);
        else if (loopKey === "patchNotes")
          items =
            state.settings.patchNotesMode === "single"
              ? context.files.slice(0, 1)
              : context.files;
        ...
        return items
          .map((item) => {
            ...
            if (
              loopKey === "groupCleanFiles" ||
              loopKey === "groupCrackedFiles" ||
              loopKey === "groupCrackedFiles2"
            ) {
              // First try exact match by platform and branch
              let mainFile = activeGame.files.find(
                (f) => f.platform === item.platform && f.branch === item.branch,
              );

              // Fallback: try finding by platform only
              if (!mainFile) {
                mainFile = activeGame.files.find(
                  (f) => f.platform === item.platform,
                );
              }

              if (mainFile) {
                // Copy sizes from main file, preferring main file's sizes but falling back to item's
                fileWithSizes.cleanFileSize =
                  mainFile.cleanFileSize || item.cleanFileSize;
                fileWithSizes.crackedFileSize =
                  mainFile.crackedFileSize || item.crackedFileSize;
                fileWithSizes.crackedFileSize2 =
                  mainFile.crackedFileSize2 || item.crackedFileSize2;
              }
            }
            ...
            let itemData = {
              ...
              group: item,
              update:
                loopKey === "updates" ? item : parentContext.update || item,
              section:
                loopKey === "sections" ? item : parentContext.section || item,
              link: item,
              ...fileWithSizes,
              ...
              showPatchNotesVersionLabel:
                state.settings.showPatchNotesVersionLabel, // Separate toggle
            };
            ...
            itemData.mainGroupTitle =
              activeGame.mainGroupTitle || "Proton Drive Links";
            itemData.cleanUrlColor = state.settings.cleanUrlColor;
            itemData.crackedUrlColor = state.settings.useSameUrlColor
              ? state.settings.cleanUrlColor
              : state.settings.crackedUrlColor;
            ...
            if (itemData.cleanFileSize)
              itemData.cleanFileSize = ensureGB(itemData.cleanFileSize);
            if (itemData.crackedFileSize)
              itemData.crackedFileSize = ensureGB(itemData.crackedFileSize);
            if (itemData.crackedFileSize2)
              itemData.crackedFileSize2 = ensureGB(itemData.crackedFileSize2);
            // Also update the nested file object for template access via {file.cleanFileSize}
            if (itemData.file) {
              if (itemData.file.cleanFileSize)
                itemData.file.cleanFileSize = ensureGB(
                  itemData.file.cleanFileSize,
                );
              if (itemData.file.crackedFileSize)
                itemData.file.crackedFileSize = ensureGB(
                  itemData.file.crackedFileSize,
                );
              if (itemData.file.crackedFileSize2)
                itemData.file.crackedFileSize2 = ensureGB(
                  itemData.file.crackedFileSize2,
                );
            }
            if (itemData.update && itemData.update.fileSize)
              itemData.update.fileSize = ensureGB(itemData.update.fileSize);
            if (itemData.fileSize)
              itemData.fileSize = ensureGB(itemData.fileSize); // Catch-all for other size props
            ...
            let processedContent = processLoops(
              loopContent,
              item,
              nestedParent,
            );
            ...
            processedContent = processedContent.replace(
              ...
              (m, k, c) => {
                ...
                if (k === "showVersionLabel" && itemData.showVersionLabel)
                  return c;
                if (
                  k === "showPatchNotesVersionLabel" &&
                  itemData.showPatchNotesVersionLabel
                )
                  return c; // Handle new separate toggle

                if (
                  k === "crackedExists" &&
                  itemData.files &&
                  itemData.files.some((f) => f.includeCracked)
                )
                  return c;
                if (k === "group.footer" && item.footer) return c;
                if (k === "patchNotesTitle" && activeGame.patchNotesTitle)
                  return c;
...
const renderPreview = (bbcode) => {
  let html = bbcode
    ...
    .replace(
      /\[url=([^\]]*)\](.*?)\[\/url\]/gs,
      (m, u, t) => `<a href="${u}" class="postlink" target="_blank">${t}</a>`,
    )
    .replace(
      /(^|\s)(https?:\/\/[^\s<]+)/g,
      '$1<a href="$2" class="postlink" target="_blank">$2</a>',
    )
    // Fixed: Use non-greedy match for spoiler title, stopping at "]
    .replace(
      /\[spoiler="([^"]*?)"\](.*?)\[\/spoiler\]/gs,
      '<div style="margin:5px 0;border:1px solid #444;padding:5px;background:#222;"><div style="font-weight:bold;color:#fff;">$1</div><div style="margin-top:5px;">$2</div></div>',
    )
    .replace(/\n/g, "<br>")
...
const createPlatformInputs = (files, parentIndex, type = "main") => {
  ...
  files.forEach((file, fIndex) => {
    const id =
      type === "group"
        ? `data-group-index="${parentIndex}" data-file-index="${fIndex}"`
        : `data-file-index="${fIndex}"`;
    const warn = file.cleanUrlNeedsUpdate
    ...
    html += `<div class="mb-3 pb-2 border-b border-gray-700 last:border-0">
            ...
            <div class="${file.includeCracked ? "mt-2" : "hidden"}">
                 <label class="block text-xs font-medium text-gray-500 mb-1">Cracked URL (Primary)</label>
            ...
            <div class="${file.includeCracked2 ? "mt-2" : "hidden"}">
                 <label class="block text-xs font-medium text-cyan-500 mb-1">Cracked URL (Secondary)</label>
                 <input type="text" ${id} data-prop="crackedUrl2" value="${file.crackedUrl2 || ""}" class="w-full p-1 bg-gray-900 border border-cyan-700 rounded-md text-sm text-cyan-300">
            </div>
        </div>`;
  });
  return html;
...
const updateUIForActiveGame = () => {
  ...
  if (state.games.length === 0) {
    els.customizationPanel.classList.add("hidden");
    renderOutput();
    return;
  }
  ...
  g.customGroups.forEach((grp, i) => {
    ...
    d.className = "p-3 bg-gray-700/30 rounded border border-gray-600 mb-3";
  ...
  state.presets.forEach((p, i) => {
    els.presetSelector.innerHTML += `<option value="${i}">${p.title}</option>`;
  });

  els.crackTogglesContainer.innerHTML = "";
  g.files.forEach((f, i) => {
    // Primary crack type detection
    ...
    const isOnlineFixSteamless =
      isOnlineFix && f.crackType.includes("Steamless");

    // Secondary crack type detection
    const isDetanup2 = f.crackType2 && f.crackType2.includes("Detanup01");
    const isOnlineFix2 = f.crackType2 && f.crackType2.includes("Online-Fix");
    const isCustom2 = f.crackType2 && !isDetanup2 && !isOnlineFix2;

    const isDetanupFork2 = isDetanup2 && !f.crackType2.includes("Steamless");
    const isDetanupSteamless2 =
      isDetanup2 && f.crackType2.includes("Steamless");
    const isOnlineFixOnly2 =
      isOnlineFix2 && !f.crackType2.includes("Steamless");
    const isOnlineFixSteamless2 =
      isOnlineFix2 && f.crackType2.includes("Steamless");

    const d = document.createElement("div");
    d.className = "p-2 bg-gray-700/30 rounded mb-2 border border-gray-600";
    ...
    d.innerHTML = `
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center">
                ...
                <label class="text-xs mr-2 text-gray-400">Primary Crack</label>
        ...
        ${
          f.includeCracked
            ...
            <select data-file-index="${i}" data-prop="crackType" class="w-full text-xs bg-gray-900 border-gray-600 rounded p-1">
                <option value="Cracked: Detanup01 Goldberg Fork" ${isDetanupFork ? "selected" : ""}>Detanup01 Goldberg Fork</option>
                <option value="Cracked: Detanup01 Goldberg Fork + Steamless" ${isDetanupSteamless ? "selected" : ""}>Detanup01 Goldberg Fork + Steamless</option>
                <option value="Cracked: Online-Fix" ${isOnlineFixOnly ? "selected" : ""}>Online-Fix</option>
                <option value="Cracked: Online-Fix + Steamless" ${isOnlineFixSteamless ? "selected" : ""}>Online-Fix + Steamless</option>
                <option value="custom" ${isCustom ? "selected" : ""}>Custom</option>
            </select>
            <input type="text" data-file-index="${i}" data-prop="customCrackType" value="${f.crackType}" class="w-full text-xs bg-gray-900 border-gray-600 rounded p-1 ${isCustom ? "" : "hidden"}">
        </div>`
            : ""
        }
        
        <!-- Secondary Crack Section -->
        ${
          f.includeCracked
            ? `
        <div class="mt-3 pt-2 border-t border-gray-600">
            <div class="flex items-center justify-between">
                <label class="text-xs text-cyan-400">Secondary Crack (Optional)</label>
            ...
            ${
              f.includeCracked2
                ? `<div class="mt-2 space-y-2">
                <select data-file-index="${i}" data-prop="crackType2" class="w-full text-xs bg-gray-900 border-gray-600 rounded p-1">
                    <option value="Cracked: Detanup01 Goldberg Fork" ${isDetanupFork2 ? "selected" : ""}>Detanup01 Goldberg Fork</option>
                    <option value="Cracked: Detanup01 Goldberg Fork + Steamless" ${isDetanupSteamless2 ? "selected" : ""}>Detanup01 Goldberg Fork + Steamless</option>
                    <option value="Cracked: Online-Fix" ${isOnlineFixOnly2 ? "selected" : ""}>Online-Fix</option>
                    <option value="Cracked: Online-Fix + Steamless" ${isOnlineFixSteamless2 ? "selected" : ""}>Online-Fix + Steamless</option>
                    <option value="custom" ${isCustom2 ? "selected" : ""}>Custom</option>
                </select>
                <input type="text" data-file-index="${i}" data-prop="customCrackType2" value="${f.crackType2 || ""}" class="w-full text-xs bg-gray-900 border-gray-600 rounded p-1 ${isCustom2 ? "" : "hidden"}">
            </div>`
                : ""
            }
        </div>`
  ...
  g.files.forEach((f, i) => {
    ...
    const hasCracked2 = f.includeCracked2;
    sizesHtml += `
        <div class="flex items-center gap-2 text-xs flex-wrap">
            ...
            ${
              hasCracked2
                ? `
            <span class="text-cyan-300 shrink-0">2nd:</span>
            <input type="text" data-file-index="${i}" data-prop="crackedFileSize2" value="${f.crackedFileSize2 || ""}" class="w-14 p-1 bg-gray-900 border border-cyan-700 rounded text-xs text-center" placeholder="GB">
            `
                : ""
            }
        </div>`;
  ...
  if (g.updates && g.updates.length > 0) {
    sizesHtml +=
      '<div class="mt-3 pt-3 border-t border-gray-700"><span class="text-xs text-gray-500 mb-2 block">Updates</span><div class="space-y-2">';
  ...
  g.updates.forEach((u, ui) => {
    ...
    if (!u.sections)
      u.sections = [
        {
          miniTitle: "Proton Drive",
          links: [{ name: "Update v1.0", url: "" }],
        },
      ];
  ...
  if (state.settings.patchNotesMode === "single" && g.files.length > 0) {
    ...
    g.files.forEach((f, i) => {
      urlContainer.innerHTML += `<div class="mb-1"><label class="text-xs text-gray-400">${f.platform}</label><input type="text" data-file-index="${i}" data-prop="patchNoteUrl" value="${f.patchNoteUrl || ""}" class="w-full bg-gray-900 border-gray-600 rounded p-1 text-sm"></div>`;
    });
  }
...
// Global Actions
window.resetMainTitle = () => {
  state.games[state.activeGameIndex].mainGroupTitle = "Proton Drive Links";
  updateUIForActiveGame();
  saveData();
};
window.addCustomGroup = () => {
  ...
  const f = g.files.map((x) => ({
    ...
    cleanUrl: "",
    crackedUrl: "",
    crackedUrl2: "", // Secondary crack URL
    shortDate: x.shortDate,
    buildId: x.buildId,
    includeCracked: x.includeCracked,
    crackType: x.crackType,
    includeCracked2: x.includeCracked2, // Secondary crack toggle
    crackType2: x.crackType2, // Secondary crack type
    // Copy existing file sizes when creating a new group
    cleanFileSize: x.cleanFileSize,
    crackedFileSize: x.crackedFileSize,
    crackedFileSize2: x.crackedFileSize2, // Secondary crack file size
  }));
  g.customGroups.push({ title: "New Group", files: f, footer: "" });
  updateUIForActiveGame();
  saveData();
};
window.removeCustomGroup = (i) => {
  if (confirm("Delete group?")) {
    state.games[state.activeGameIndex].customGroups.splice(i, 1);
    updateUIForActiveGame();
    saveData();
  }
};
window.loadPreset = () => {
  ...
  const p = state.presets[idx],
    g = state.games[state.activeGameIndex],
    f = g.files.map((x) => ({
      platform: x.platform,
      branch: x.branch,
      cleanUrl: "",
...
window.savePreset = () => {
  const t = prompt("Name:");
  if (!t) return;
  const f = prompt("Footer:");
  state.presets.push({ title: t, footer: f || "" });
  updateUIForActiveGame();
  saveData();
};
...
window.addUpdate = () => {
  ...
  const defaultFilename = generateUpdateFilename(
    game.originalTitle,
    defaultTitle,
  );
  ...
  state.games[state.activeGameIndex].updates.push({
    ...
    sections: [
      {
        miniTitle: "Proton Drive",
        links: [{ name: defaultFilename, url: "" }],
      },
    ],
  });
  updateUIForActiveGame();
...
window.removeUpdate = (i) => {
  if (confirm("Delete update?")) {
    state.games[state.activeGameIndex].updates.splice(i, 1);
    updateUIForActiveGame();
    saveData();
  }
};
...
window.addUpdateSection = (uIndex) => {
  ...
  state.games[state.activeGameIndex].updates[uIndex].sections.push({
    miniTitle: "New Provider",
    links: [{ name: filename, url: "" }],
  });
  updateUIForActiveGame();
  saveData();
};
window.removeUpdateSection = (uIndex, sIndex) => {
  if (confirm("Remove this provider section?")) {
    state.games[state.activeGameIndex].updates[uIndex].sections.splice(
      sIndex,
      1,
    );
    updateUIForActiveGame();
...
window.addSectionLink = (uIndex, sIndex) => {
  ...
  state.games[state.activeGameIndex].updates[uIndex].sections[
    sIndex
...
window.copyCrackFileName = (fileIndex) => {
  ...
  const title = game.originalTitle
    .replace(/[^\w\s-&]/g, "")
    .trim()
    .replace(/\s+/g, ".");
...
function setupEventListeners() {
  ...
  els.processTextBtn.addEventListener("click", () => {
    if (els.textInput.value.trim()) parseInputText(els.textInput.value);
  });
  ...
  els.customizationPanel.addEventListener("input", (e) => {
    ...
    if (t.dataset.setting === "patchNotesTitle") {
      game.patchNotesTitle = t.value;
      saveData();
      renderOutput();
      return;
    }
    ...
    if (set) {
      if (set === "patchNotesMode") {
        state.settings.patchNotesMode = t.checked ? "single" : "multiple";
        updateUIForActiveGame();
      } else if (set === "showVersionLabel") {
        state.settings.showVersionLabel = t.checked;
        updateUIForActiveGame();
      } // Handle Top Version Label Toggle
      else if (set === "showPatchNotesVersionLabel") {
        state.settings.showPatchNotesVersionLabel = t.checked;
        updateUIForActiveGame();
      } // Handle Patch Notes Version Label Toggle
    ...
    const f = t.dataset.fileIndex,
      g = t.dataset.groupIndex,
      u = t.dataset.updateIndex,
      s = t.dataset.sectionIndex,
      l = t.dataset.linkIndex,
      p = t.dataset.prop;
    ...
    if (p) {
      ...
      // UPDATE LOGIC FOR NEW STRUCTURE
        ...
        if (p === "linkName")
          ...
          game.updates[u].sections[s].links[l].url = t.value;
      } else if (u !== undefined && s !== undefined && p === "miniTitle")
        game.updates[u].sections[s].miniTitle = t.value;
        ...
        const oldFilename = generateUpdateFilename(
          game.originalTitle,
          oldTitle,
        );
        const newFilename = generateUpdateFilename(
          game.originalTitle,
          newTitle,
        );
        ...
        if (p === "includeCracked") {
          ...
          if (val === "custom") {
            t.closest("div")
              .querySelector('[data-prop="customCrackType"]')
    ...
    if (t.type === "color" || t.id === "use-same-url-color") {
      if (t.id === "use-same-url-color") {
        state.settings.useSameUrlColor = t.checked;
        updateUIForActiveGame();
      }
      if (t.id === "clean-url-color") state.settings.cleanUrlColor = t.value;
      if (t.id === "cracked-url-color")
        state.settings.crackedUrlColor = t.value;
      if (t.id === "section-title-color")
        state.settings.sectionTitleColor = t.value;
    }
    renderOutput();
    saveData();
  });

  els.simpleModeBtn.addEventListener("click", () => {
    els.simpleModeBtn.classList.add("active");
    els.advancedModeBtn.classList.remove("active");
    els.simpleModeControls.classList.remove("hidden");
    els.advancedModeControls.classList.add("hidden");
  });
  els.advancedModeBtn.addEventListener("click", () => {
    els.advancedModeBtn.classList.add("active");
    els.simpleModeBtn.classList.remove("active");
    els.advancedModeControls.classList.remove("hidden");
    els.simpleModeControls.classList.add("hidden");
  });
  els.previewTabBtn.addEventListener("click", () => {
    els.previewTabBtn.classList.add("active");
    els.codeTabBtn.classList.remove("active");
    els.previewPane.classList.remove("hidden");
    els.codePane.classList.add("hidden");
  });
  els.codeTabBtn.addEventListener("click", () => {
    els.codeTabBtn.classList.add("active");
    els.previewTabBtn.classList.remove("active");
    els.codePane.classList.remove("hidden");
    els.previewPane.classList.add("hidden");
  });
  els.copyBtnTop.addEventListener("click", () =>
    handleCopyClick(els.copyBtnTop),
  );
  els.copyBtnBottom.addEventListener("click", () =>
    handleCopyClick(els.copyBtnBottom),
  );
  els.downloadBtn.addEventListener("click", () => {
    const blob = new Blob([els.outputCode.value], { type: "text/plain" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `${state.games[state.activeGameIndex].gameTitle}.txt`;
    a.click();
  });
  els.templateEditor.addEventListener("input", (e) => {
  ...
  els.registerButton.addEventListener(
    "click",
    () => (els.registerModal.style.display = "block"),
  );
  els.logoutButton.addEventListener("click", () => handleLogout());
  els.localSyncButton.addEventListener(
    "click",
    () => (els.syncModal.style.display = "block"),
  );
  ...
  if (els.changePasswordButton)
    els.changePasswordButton.addEventListener(
      "click",
      () => (els.changePasswordModal.style.display = "block"),
    );
  if (els.changePasswordForm)
    els.changePasswordForm.addEventListener("submit", handleChangePassword);
  document.body.addEventListener("click", (e) => {
    if (e.target.matches(".close-modal, .close-auth-modal, .close-sync-modal"))
...
function updateUIForLoginState() {
  const els = getElements();
  if (!els.loginButton || !authManager) return;
  ...
  els.userStatus.textContent = loggedIn
    ? `Logged in: ${currentUser?.username || "User"}`
    : "Not logged in";
  els.userStatus.style.color = loggedIn ? "#4bc0c0" : "#ccc";
  if (els.changePasswordButton)
    els.changePasswordButton.style.display = loggedIn ? "inline-block" : "none";
}
...
async function handleLogin(e) {
  ...
  try {
    await authManager.login(
      els.loginForm.loginUsername.value,
      els.loginForm.loginPassword.value,
    );
...
async function handleRegister(e) {
  ...
  if (
    els.registerForm.registerPassword.value !==
    els.registerForm.registerConfirmPassword.value
  ) {
    els.registerError.textContent = "Mismatch";
    return;
  }
  try {
    await authManager.register(
      els.registerForm.registerUsername.value,
      els.registerForm.registerPassword.value,
    );
    alert("Registered.");
    els.registerModal.style.display = "none";
    els.loginModal.style.display = "block";
  } catch (err) {
...
async function handleChangePassword(e) {
  ...
  if (
    els.changePasswordForm.newPassword.value !==
    els.changePasswordForm.confirmNewPassword.value
  ) {
    els.changePasswordError.textContent = "Mismatch";
    return;
  }
  try {
    await authManager.changePassword(
      els.changePasswordForm.currentPassword.value,
      els.changePasswordForm.newPassword.value,
    );
    alert("Changed.");
    await handleLogout();
  } catch (err) {
    els.changePasswordError.textContent = err.message;
  }
}
  ...
  if (!game.customGroups || game.customGroups.length === 0) {
    ...
    game.customGroups.forEach((grp, gi) => {
      ...
      grp.files.forEach((f, fi) => {
        ...
        const mainFile = game.files.find(
          (mf) => mf.platform === f.platform && mf.branch === f.branch,
        );
        if (mainFile) {
          ...
          console.log(
            `      -> Main file NOT FOUND for ${f.platform}/${f.branch}`,
          );
          // Try platform-only match
          const platformMatch = game.files.find(
            (mf) => mf.platform === f.platform,
          );
          if (platformMatch) {
            console.log(
              `      -> Platform-only match found: ${platformMatch.platform}/${platformMatch.branch}`,
            );
          }
        }
      });
...
window.syncFileSizesNow = () => {
  ...
  game.customGroups.forEach((grp) => {
    grp.files.forEach((cgFile) => {
      const mainFile = game.files.find(
        (mf) => mf.platform === cgFile.platform && mf.branch === cgFile.branch,
      );
      if (mainFile) {
