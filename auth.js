...
// UPDATED: Fixed "Token Fighting" race conditions and added cross-tab sync.
...
class AuthManager {
    ...
    constructor(appName, environment = 'wip') {
        ...
        // This listens for changes made by OTHER tabs. 
        // If Tab A refreshes the token, Tab B updates its variables instantly.
        window.addEventListener('storage', (event) => {
            if (event.key === `${this.authStoragePrefix}authToken`) {
                ...
                // Optional: If we went from no user to user, dispatch login
            ...
            // If tokens are removed (logout in another tab), update state
    ...
    _decodeJwtPayload(token) {
        try {
            ...
            if (!base64Url) return null; // Handle invalid tokens gracefully
    ...
    /**
     * Checks if a JWT is expired or close to expiring (buffer of 60 seconds).
     */
    isTokenExpired(token) {
        ...
        try {
            ...
            // Consider expired if within 60 seconds of expiry (increased buffer)
    ...
    async attemptRefreshToken() {
        // Always grab the latest from storage just in case sync was slightly delayed
        this.refreshToken = localStorage.getItem(`${this.authStoragePrefix}refreshToken`);
        ...
        try {
            ...
            // Some backends rotate refresh tokens, some don't. 
            // If a new one is sent, save it. Otherwise keep the old one.
            ...
            // Only logout if it was a genuine auth error (401/403), not a network glitch
            // But for simplicity, we assume refresh failure = session death
    ...
    async login(username, password) {
        ...
        // Saving to localStorage triggers 'storage' event in other tabs
        localStorage.setItem(`${this.authStoragePrefix}authToken`, this.authToken);
        localStorage.setItem(`${this.authStoragePrefix}refreshToken`, this.refreshToken);
    ...
    async logout(message = null) {
        ...
        // This removal triggers 'storage' event in other tabs, logging them out too
     ...
     * UPDATED: Now smart enough to NOT refresh if the token is still valid.
     */
    async initialize() {
        this._log("Initializing session...");
        
        if (this.isLoggedIn()) {
            // CHECK 1: Is the token actually expired?
            if (!this.isTokenExpired(this.authToken)) {
            ...
            // CHECK 2: Only refresh if strictly necessary
